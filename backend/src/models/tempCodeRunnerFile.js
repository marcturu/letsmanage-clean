...
export class ModelExercise{
    ...
    static async getAllByFilters({ type, personalized }) {
        try {
          ...
          if (type) {
            exercises = await new DatabaseClass().obtainDataCollectionByField('exercises', 'type', type);
          }
    
          if (personalized !== undefined) {
            // Esperamos que los ejercicios personalizados se obtengan correctamente
            const personalizedExercises = await new DatabaseClass().obtainDataCollectionByField('exercises', 'personalized', personalized);
